...
predict_ambient = function (s)
{
    ...
    for (let i = 0; i < last_ambs.length; i++)
        cnt[last_ambs[i][1]]++;
    let max_cnt = 0, max_src = 'nothing', max_sent = 'nothing';
    for (let i = 0; i < last_ambs.length; i++)
    {
    ...
    if (max_src !== prev_chosen_amb_src)
    {
        ...
        (+new Date) - popup_delay >= prev_chosen_amb_ts[max_src] &&
        !translated_sent[max_sent])
    {
        console.log('MAX SENT', max_sent);
        console.log('TRANSLATION', ru_to_tat(max_sent)); // to pop up
